...
const VenueOwnerNavLayout = () => {
  ...
  const navigationItems = [
    { name: 'Dashboard', icon: MdDashboard, href: '/venue-owner' },
    { name: 'Venues', icon: MdLocationOn, href: '/venue-owner/venues' },
    { name: 'Bookings', icon: MdCalendarToday, href: '/venue-owner/bookings' },
  ]

  const profileMenuItems = [
    { name: 'Settings', href: '/venue-owner/settings', action: () => navigate('/venue-owner/settings') },
    { name: 'Help & Support', href: '/venue-owner/help', action: () => navigate('/venue-owner/help') },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
              ...
              {navigationItems.map((item) => {
                ...
                return (
                  <a
                    key={item.name}
                    href={item.href}
                    className="group relative flex items-center space-x-2 text-gray-700 hover:text-green-600 px-4 py-2 text-sm font-medium transition-all duration-300 cursor-pointer"
                  >
                    <Icon className="w-5 h-5 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300" />
                    <span className="group-hover:translate-x-1 transition-transform duration-300">{item.name}</span>
                    <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-green-600 rounded-full group-hover:w-full transition-all duration-300"></div>
                  </a>
                )
              })}
            </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            {/* Notifications */}
            <button className="group relative p-2 text-gray-700 hover:text-green-600 transition-all duration-300 cursor-pointer">
            ...
            <div className="relative">
              <button
                ...
                className="group flex items-center space-x-2 text-gray-700 hover:text-green-600 transition-all duration-300 cursor-pointer"
              >
                ...
                <span className="text-sm font-medium group-hover:translate-x-1 transition-transform duration-300">Venue Owner</span>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl border border-gray-200 py-2 z-50 animate-in slide-in-from-top-2 duration-300">
                  {profileMenuItems.map((item, index) => (
                    <button
                      ...
                      className="group flex items-center px-4 py-3 text-sm text-gray-700 hover:text-green-600 transition-all duration-300 cursor-pointer relative w-full text-left"
          ...
          <div className="md:hidden">
            <button
              ...
              className="group inline-flex items-center justify-center p-2 text-gray-700 hover:text-green-600 transition-all duration-300 cursor-pointer"
            >
              {isMobileMenuOpen ? (
            ...
            {navigationItems.map((item, index) => {
              ...
              return (
                <a
                  key={item.name}
                  href={item.href}
                  className="group flex items-center space-x-3 text-gray-700 hover:text-green-600 block px-3 py-3 text-base font-medium transition-all duration-300 cursor-pointer"
...
export default VenueOwnerNavLayout
